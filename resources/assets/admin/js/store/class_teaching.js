...
const actions = {
    query(context, classInformationId) {
        return Vue.http.get(`${ADMIN_CONFIG.ADMIN_URL}/classinformation/${classInformationId}/teachings`)
            .then(response => {
                context.commit('set', response.data);
            })
    },
    store(context, {teacherId, subjectId, classInformationId}) {
        return ClassTeaching.save({classinformation: classInformationId},
    ...
    destroy(context, {teachingId, classInformationId}) {
        return ClassTeaching.delete({classinformation: classInformationId, teaching: teachingId})
            .then(() => {
